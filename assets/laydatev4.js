...
!(function (win) {
    ...
    */
    ...
    lay.position = function (elem, elemView, obj) {
      ...
      //设置坐标
      elemView.style.left =
        left + (position === "fixed" ? 0 : scrollArea(1)) + "px";
  ...
  !(function (window, document) {
    ...
    var isLayui = window.layui && layui.define,
      ...
      laydate = {
        ...
        ready: function (fn) {
          var cssname = "laydate",
            ...
            path =
              // 改css
              "https://cdn.shopify.com/s/files/1/0583/5810/4213/files/laydatev3.css";
           //"../src/theme/default/laydatev3.css?v=" + laydate.v + ver;
          isLayui
      ...
      ELEM_INFOFOOTER = "layui-laydate-info-footer",
      ELEM_OUTSIDEHEADER = "layui-laydate-outside-header",
      ELEM_SIDE = "layui-laydate-side",
    ...
    */
    ...
    Class.prototype.config = {
      ...
      showTop: true, //改-新增是否显示顶栏
      showSide: true, //改-新增是否显示右侧时间选择栏
      specific: 0, //改-新增特殊
      isSpecDay: false, //改
      TimePeriodization:0,
      submitInfoState: 0, //改-是否提交
      showInfoBottom: true, //改-新增是否显示信息底部栏,
      shopList: {
        /*测试 */
        /*
        "215-235-1838": {
          ...
          timezone: "Philadelphia, PA, USA (GMT-4)",
          testRideSize:"M",
    ...
    Class.prototype.init = function () {
      ...
      //改-popupID
      that.elemPopupID = "booking-popup" + options.elem.attr("lay-key");
    ...
    Class.prototype.render = function () {
      var that = this,
        ...
        if(options.positionPlus=="static"){
          bookingPopup += " booking-popup-static"
          popupCenter += " popup-center-static"
        }
        //主面板
        //改-外部
        let layuiLaydateStyle = "width:400px"
        if(options.positionPlus == "static"){
          layuiLaydateStyle="width:50%;"
        }
        var elemPopup = (that.elemPopup = lay.elem("div", {
          class: bookingPopup,//"booking-popup",
        })),
        
        elemCenter = (that.elemCenter = lay.elem("div", {
          class: popupCenter,//"popup-center",
        })),
        elem = (that.elem = lay.elem("div", {
          ...
          style: layuiLaydateStyle,
        })),
        //改2-选择商店区域
        elemSelectShop = (that.elemSelectShop = lay.elem("div", {
          class: "shop-select-content",
        })),
        //改-商店展示区域
        elemShop = (that.elemShop = []),
        //主区域
        ...
        //改-日历区域
        elemCalender = (that.elemCalender = lay.elem("div", {
          class: "calendar-content",
        })),
        //改-顶部区域
        divOutsideHeader = (that.footer = lay.elem("div", {
          class: ELEM_OUTSIDEHEADER,
        })),
        //底部区域
        ...
        //改-新信息底部区域
        divInfoFooter = (that.infoFooter = lay.elem("div", {
          class: ELEM_INFOFOOTER,
        })),
        //改-侧边区域
        ...
        var closebtnClass = "close-btn fa-solid fa-xmark";
        if(options.positionPlus == "static"){
          closebtnClass+= " close-btn-static"
        }
        var divCloseBtn = (that.closeBtn = lay.elem("i", {
          class: closebtnClass,
        }));
      //改2-
      if (Object.keys(options.shopList).length > 1 && options.specific == 0) {
        //that.step = 1; //先进入步骤0,选择店铺
        that.haveStep0 = true;
        elemCenter.appendChild(elemSelectShop);
        Object.keys(options.shopList).forEach((item) => {
          let node = lay.elem("div", { class: "shop-list-item" });
          //改2
  
          let divShopName = lay.elem("div", {
            class: "shop-name",
          });
          ...
          lay(divShopDetail).html(
            (function () {
              var html = [];
              html.push(
                '<div class="info-title">Test ride bike - Carbon E-Bike Size ' +
                  options.shopList[item].testRideSize +
                  "</div>"
              );
              html.push(
                '<div class="info-title">ADD: <span>' +
                  options.shopList[item].add +
                  "</span></div>"
              ); /*
              html.push(
                '<div class="info-title">Email: <span>' +
                  options.shopList[item].email +
                  "</span></div>"
              );
              html.push(
                '<div class="info-title">Phone: <span>' +
                  options.shopList[item].phone +
                  "</span></div>"
              );*/
          ...
          let divShopSelectBtn = lay.elem("button", {
            class: "shop-select-btn",
          });
          lay(divShopSelectBtn).html("Select");
          lay(divShopSelectBtn).on("click", function () {
            options.shopInfo = options.shopList[item];
            that.next();
          });
          node.appendChild(divShopName);
          node.appendChild(divShopImg);
          node.appendChild(divShopDetail);
          node.appendChild(divShopSelectBtn);
          elemSelectShop.appendChild(node);
        });
      } else if (
        Object.keys(options.shopList).length > 0 &&
        options.specific == 0
      ) {
        that.haveStep0 = false;
        options.shopInfo = options.shopList[Object.keys(options.shopList)[0]];
      } else if (options.specific >= 1) {
        that.haveStep0 = false;
        options.shopInfo = options.shopList[Object.keys(options.shopList)[0]];
      }
      //改-
      that.closeCheck = function (e) {
        let t = document.querySelector(".popup-center");
        var force = false;
        if (this != document) {
          force = lay(this).attr("class").indexOf("close-btn") != -1;
        }
        if ((!e.target.closest(".popup-center") && t) || force) {
          ...
          if (name && phone && email && that.submitInfoState == 0) {
            let body = {
              trace_name: "testride-bookNow-exitInfo",
              extras: {
          ...
          divCloseBtn.removeEventListener("click", that.closeCheck);
        }
      };
      lay(divCloseBtn).on("click", that.closeCheck);
      document.addEventListener("mousedown", that.closeCheck);
      var mo = function (e) {
        passive: false;
      };
      let bookingShopContentStyle = "width:370px";
      let bookingShopContentClass = "booking-shop-content";
      ///////////////////////
      if(options.positionPlus !="static"){
        ...
        bookingShopContentStyle="width:50%";
        bookingShopContentClass += " booking-shop-content-static";
      }
      elemShop[0] = lay.elem("div", {
        class: bookingShopContentClass,
        style: bookingShopContentStyle,
      });
      //改-插入左侧商店区域
      lay.each(elemShop, function (i, main) {
        elemCenter.appendChild(main);
      });
      elemPopup.appendChild(elemCenter);
      elemCenter.appendChild(elem);
      elemCenter.appendChild(divCloseBtn);
      ...
      //生成顶部栏
      lay(divOutsideHeader).html(
        (function () {
          var html = [],
            btns = [];
          html.push(
            '<button class="back-btn hidden-btn"><i class="fa-solid fa-arrow-left"></i></button>'
          );
          html.push(
            '<button class="back-btn-mb hidden-btn"><i class="fa-solid fa-arrow-left"></i></button>'
          );
          html.push('<div class="select-tip">Select a Date<div>');
          return html.join("");
        })()
      );
      //生成底部栏
      ...
      //改-新增信息底部栏
      lay(divInfoFooter).html(
        (function () {
          var html = [];
          html.push(
            '<div class="time-zone-tip">The appointment time is based on the store\'s timezone</div>' +
              '<div class="time-zone"><i class="fa-solid fa-earth-americas"></i><span>' +
      ...
      lay(divSide).html(
        (function () {
          ...
          html.push('<div class="laydate-side-btns"></div>');
          return html.join("");
        })()
      );
      ...
      lay.each(elemMain, function (i, main) {
        elemCalender.appendChild(main);
      });
      options.showBottom && elem.appendChild(divFooter);
      //改-新增顶边
      options.showTop && elem.appendChild(divOutsideHeader);
      //改-新增侧边
      options.showSide && elemCalender.appendChild(divSide);
      options.showSide && elem.appendChild(elemCalender);
      //改-新增信息底部栏
      options.showInfoBottom && elem.appendChild(divInfoFooter);
      //生成自定义主题
      ...
      //改-需要改为外部的elmepopup
      isStatic
        ? options.elem.append(elemPopup)
        : document.body.appendChild(elemPopup); //that.position()); //定位 //改-pop
      ...
      if (options.specific >= 1) {
        //改-bike-attack
        let scy = 37 - window.scrollY;
        if (scy > 0 && window.innerWidth < 750) {
          elemPopup.style.height = "calc(100% - " + (69 + scy) + "px)";
          elemPopup.style.top = 69 + scy + "px";
        }
      }
    ...
    Class.prototype.checkDate = function (fn) {
      ...
      //校验主面板是否在可选日期区间
      if (
        getDateTime(dateTime) > getDateTime(options.max) ||
        getDateTime(dateTime) < getDateTime(options.min)
      ) {
      ...
      if (options.range) {
        if (
          getDateTime(that.endDate) < getDateTime(options.min) ||
          getDateTime(that.endDate) > getDateTime(options.max)
        ) {
      ...
      //test
      that.bookingDate = JSON.parse(JSON.stringify(options.dateTime));
      fn && fn();
      return that;
    };
    ...
    Class.prototype.calendar = function (value, index, type) {
      ...
      if (dateTime.year < LIMIT_YEAR[0])
        (dateTime.year = LIMIT_YEAR[0]), that.hint(lang.invalidDate);
      if (dateTime.year > LIMIT_YEAR[1])
        (dateTime.year = LIMIT_YEAR[1]), that.hint(lang.invalidDate);
      ...
      lay.each(tds, function (index_, item) {
        var disabledDays = [];
        lay.each(options.shopInfo.businessHours, function (i, item1) {
          if (item1 == "") {
            disabledDays.push(i);
          }
        });
        ...
        lay.each(disabledDays, function (i, item2) {
          //改-特殊时间
          if (index_ % 7 == item2) {
            item.addClass("disabled-day");
            item.css("cursor", "no-drop");
          }
        });
        if (index_ < startWeek) {
          st = prevMaxDate - startWeek + index_;
          item.addClass(ELEM_PREV);
          YMD = that.getAsYM(dateTime.year, dateTime.month, "sub");
        } else if (index_ >= startWeek && index_ < thisMaxDate + startWeek) {
          st = index_ - startWeek;
          //改-换页不换时间
          dateTime.year == that.bookingDate.year &&
            dateTime.month == that.bookingDate.month &&
            st + 1 === dateTime.date &&
            item.addClass(THIS);
        } else {
        ...
        var specdisabledDay2022 = [22, 23, 24, 25, 26, 27,31];
        var specdisabledDay2023 = [1];
        var isspec2022 =
          options.isSpecDay &&
          dateTime.year == 2022 &&
          dateTime.month == 11 &&
          specdisabledDay2022.includes(st + 1);
        var isspec2023 =
          options.isSpecDay &&
          dateTime.year == 2023 &&
          dateTime.month == 0 &&
          specdisabledDay2023.includes(st + 1);
        if (isspec2022 || isspec2023) {
          item.addClass("disabled-day");
          item.css("cursor", "no-drop");
        }
        YMD[1]++;
      ...
      if (type === "year" || type === "month") {
        ...
        lay(ul)
          ...
          .on("click", function () {
            ...
            //当为年选择器或者年月选择器
            var isYearOrMonth =
              options.type === "year" || options.type === "month";
            if (isYearOrMonth) {
              lay(ul)
                .find("." + THIS)
                .removeClass(THIS);
    ...
    Class.prototype.next = function () {
      ...
      if (that.step == 1) {
        ...
        lay(
          ".booking-shop-content .shop-detail .info-title:nth-child(1) span"
        ).html(options.shopInfo.testRideSize);
        lay(
          ".booking-shop-content .shop-detail .info-title:nth-child(2) span"
        ).html(options.shopInfo.add);
        lay(
          ".booking-shop-content .shop-detail .info-title:nth-child(3) span"
        ).html(options.shopInfo.email);
        lay(
        ...
        var divBookingForm = lay.elem("div", {
            class: "booking-info-form",
          }),
          divBookingFormContent = lay.elem("div", {
            class: "form-content",
          }),
          divBookingFormSubmit = lay.elem("div", {
            class: "form-button",
          }),
          divBookingTimeInfo = lay.elem("div", {
            class: "info-title booking-time",
          });
        lay(divBookingFormContent).html(
          (function () {
            var html = [];
            html.push(
              '<div class="form-name"><div>Name*</div><input type="text" name="name"></input></div>'
            );
            html.push(
              '<div class="form-phone"><div class="form-title-phone">Phone*</div><input type="text" name="phone_number"></input></div>'
            );
            html.push(
              '<div class="form-email"><div class="form-title-email">Email*</div><input type="text" name="email"></input></div>'
            );
            return html.join("");
          })()
        );
        if (options.specific == 1) {
          lay(divBookingFormSubmit).html(
            (function () {
              var html = [];
              html.push('<button class="submit-btn">Book Now</button>');
              return html.join("");
            })()
          );
        } else {
        ...
        if (options.specific >= 1) {
          //改-bike-attack
          lay(".booking-shop-content .shop-name").html(
            "Test ride event registration"
          );
          lay(".shop-detail .info-title:nth-child(1)").remove();
          if (options.specific == 1) {
            lay(divBookingTimeInfo).html(
              (function () {
                var html = [];
                html.push("Event time: " + "<span>2022-12-17 11:00-15:00</span>");
                return html.join("");
              })()
            );
            ...
            lay(divBookingTimeInfo).html(
              (function () {
                var html = [];
                html.push("Event time: " + "<span>2022-12-17 09:00-17:00</span>");
                return html.join("");
              })()
            );
          }
          lay(".booking-shop-content").append(divBookingTimeInfo);
        }
  
        that.divBookingForm = divBookingForm;
        lay(".select-tip").addClass("hidden-btn");
        ...
        if(options.positionPlus !="static"){
          ...
          lay(".booking-shop-content")[0].innerHTML=that.divBookingForm.outerHTML;
  
          that.next();
        }
        lay(".booking-info-form .submit-btn").on("click", function () {
          var btn = this;
          var name = lay(".form-name input").val(),
            phone = lay(".form-phone input").val(),
            email = lay(".form-email input").val(),
          ...
          var appendTip = function (nodeStr, tipContent) {
            if (!lay(nodeStr).find(".form-input-tip").length) {
              var it = lay.elem("div", { class: "form-input-tip" });
              it.innerHTML = tipContent;
              lay(nodeStr).append(it);
            }
          };
          var valid = function () {
            var v1 = false,
            ...
            if (/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(email)) {
              v3 = true;
              lay(".form-email").find(".form-input-tip").remove();
            } else {
              appendTip(".form-email", "Please input the correct email address.");
            }
            if (!/^\s*$/g.test(name)) {
              v1 = true;
              lay(".form-name").find(".form-input-tip").remove();
            } else {
              appendTip(".form-name", "Can't be blank.");
            }
            if (/^[0-9]{7,15}$/.test(phone)) {
              v2 = true;
              lay(".form-phone").find(".form-input-tip").remove();
            } else {
              appendTip(".form-phone", "Please input the correct Phone Number.");
            }
          ...
          if (valid()) {
            let body = {
              trace_name: "testride-bookNow-submitInfo",
              extras: {
                spot: that.config.shopInfo.testrideSpot,
            ...
            if(options.positionPlus == "static"){
              body.extras.from = "leadform";
              if(that.config.shopInfo.name.indexOf("leadform")==-1){
                that.config.shopInfo.name +="-leadform";
              }
              var data = {
                ...
                source: window.location.host,
                book_time: that.selectedTime,
                shop_info: that.config.shopInfo,
              };
      ...
      //预约成功
        ...
        var divBookingSuccess = lay.elem("div", {
          ...
          divBookingSusscessTitle = lay.elem("div", {
            class: "success-title",
          }),
          divBookingSusscessContent = lay.elem("div", {
        ...
        lay(divBookingSusscessButton).html(
          (function () {
            var html = [];
            html.push('<button class="done-btn">Done</button>');
            return html.join("");
          })()
        );
        that.divBookingSuccess = divBookingSuccess;
        lay(".back-btn").addClass("hidden-btn");
        divBookingSuccess.appendChild(divBookingSusscessTitle);
        divBookingSuccess.appendChild(divBookingSusscessContent);
        divBookingSuccess.appendChild(divBookingSusscessButton);
        that.elemCenter.appendChild(divBookingSuccess);
        lay(".booking-success .done-btn").on("click", function () {
          if(that.config.positionPlus == "static")
          {
    ...
    Class.prototype.prev = function () {
      ...
      if (that.step == 2 && window.innerWidth > 750) {
        that.step -= 1;
      }
      if (that.step == 0) {
        that.divBookingForm.remove(); //移除bookinfo
        lay(".shop-select-content").css("display", "flex");
      } else if (that.step == 1) {
        if (!that.haveStep0) {
          lay(".back-btn-mb").addClass("hidden-btn");
          lay(".back-btn").addClass("hidden-btn");
        }
        //lay(".select-tip").addClass("hidden-btn");
        that.divBookingForm.style.visibility = "visible";
        if (
          that.config.showSide &&
          lay("." + ELEM_SIDE).css("display") == "block"
        ) {
          var wid = Number(that.elem.style.width.replace("px", "")) - 200;
          wid += "px";
          that.elem.style.width = wid;
        }
        lay("." + ELEM_SIDE).css("display", "none");
        lay(".layui-laydate-side").css("display", "none");
        lay(".select-tip").html("Select a Date");
      } else if (that.step == 2) {
        if (!that.haveStep0) {
          lay(".back-btn").addClass("hidden-btn");
        }
        //lay(".booking-time").remove();
        //that.elemCalender.removeChild(that.divBookingForm);
      }
    };
    //记录列表切换后的年月
    ...
    Class.prototype.setBtnStatus = function (tips, start, end) {
      ...
      if (options.range && options.type !== "time") {
        ...
        if (tips && isOut)
          that.hint(
            typeof tips === "string"
              ? lang.timeout.replace(/日期/g, tips)
              : lang.timeout
          );
      }
    };
    ...
    //执行 done/change 回调
    Class.prototype.done = function (param, type, tri) {
      var that = this,
        options = that.config,
    ...
    //改-选择侧边栏时间
    Class.prototype.chooseSideTime = function (btn, type) {
      var that = this,
      options = that.config;
      lay(".layui-timeTHIS").removeAttr("disabled");
      lay(".layui-timeTHIS").removeClass("layui-timeTHIS");
      lay(".layui-timeTHIS-static").removeClass("layui-timeTHIS-static");
      lay(btn).addClass("layui-timeTHIS");
      lay(btn).attr("disabled", "");
      if(options.positionPlus=="static"){
        lay(btn).addClass("layui-timeTHIS-static");
        that.bookingTime = btn.parentNode.firstChild.innerHTML;
        that.selectedTime=lay(".layui-laydate-preview")[0].innerHTML + " "+that.bookingTime
      }
    };
    //底部按钮
      ...
      var that = this,
        ...
        addSubYeay = function (type) {
          ...
          if (isYear || isMonth) {
            ...
            options.range || that.done(null, "change", "ym");
            options.range ||
  ...
  */
      ...
      lay.each(that.table, function (i, table) {
        ...
        tds.on("click", function (index) {
          if (
            this.getAttribute("class").indexOf("laydate-disabled") != -1 ||
            this.getAttribute("class").indexOf("disabled-day") != -1
          )
            return;
          ...
          if (options.TimePeriodization==1) {
            ...
            if (timeSnipt != "") {
              ...
              startHour = time[0].split(":")[0];
              startMin = time[0].split(":")[1];
              endHour = time[1].split(":")[0];
              endMin = time[1].split(":")[1];
            }
            for (var i = Number(startHour); i <= Number(endHour); ) {
              let h = i.length == 1 ? "0" + i : i;
              /*
          ...
          /*
           */
          lay.each(timeList, function (i, item) {
            timeListHtml.push(
              '<div lay-type="timeList" class="laydate-side-btn">' +
                '<button class="laydate-timeList-btn laydate-timeList-btn' +
                i +
                '">' +
                item +
                "</button>" +
                '<button class="laydate-timeList-confirm">confirm</button>' +
                "</div>"
            );
          });
          lay(".laydate-side-btns").html(timeListHtml.join(""));
  
          //改-点击侧边栏按钮
          lay(that.side)
            .find("div > button:nth-child(1)")
            .on("click", function () {
              that.chooseSideTime(this);
            });
          lay(that.side)
          ...
          if (options.showSide && lay("." + ELEM_SIDE).css("display") == "none") {
            ...
            lay("." + ELEM_SIDE).css("display", "block");
            let body = {
              trace_name: "testride-bookNow-chooseDate",
              extras: {
                spot: that.config.shopInfo.testrideSpot,
                userInfo: that.userInfo,
              },
      ...
      //改-点击返回
      lay(".layui-laydate .back-btn").on("click", function () {
        that.prev();
      });
      lay(".layui-laydate .back-btn-mb").on("click", function () {
        if(options.positionPlus=="static"){
          that.step=2;
        }
        that.prev();
